...
function AdicionarWATTS(){
    const storedItems = JSON.parse(localStorage.getItem('selectedItems')) || {};
    WATTS = 0;
    componentes.forEach(componente => {
        if (storedItems[componente]) {
            if(componente!='FONTE'){
                if(storedItems[componente].watts==null){
                    WATTS+=50;
                }
                else{
                    WATTS+=storedItems[componente].watts;
                }
            }
        }
    });
...
function mostrarNaTabela(Componente, tipo) {
    if (Componente != '' && tipo != ''){
        ...
        if(Componente.desc != ''){
            document.getElementById(tipo + '-desc').innerText = Componente.desc;
        }
...
function RemoverComponenteTabela(tipo) {
    ...
    if (Info != '' && Preco != '') {
        ...
        if (storedItems[tipo]) {
            ...
            document.getElementById(tipo + '-desc').innerText = '';
